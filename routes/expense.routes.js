...
router.post('/expenses', isAuthenticated, async (req, res, next) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: `Something went wrong creating expense: ${error.message}`,
      });
  }
});
...
router.get('/expenses/:expenseId', isAuthenticated, async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: `Something went wrong getting one expense from DB: ${error.message}`,
      });
  }
});
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      let response = await Expense.findByIdAndUpdate(expenseId, req.body, {
        new: true,
      });
      ...
      res
        .status(500)
        .json({
          message: `Something went wrong updating expense: ${error.message}`,
        });
    }
  },
);
  ...
  async (req, res, next) => {
    try {
      ...
      res
        .status(200)
        .json({
          message: `Expense with id: ${expenseId} was deleted and corresponding month expenses successfully updated.`,
        });
    } catch (error) {
      res
        .status(500)
        .json({
          message: `Something went wrong deleting expense: ${error.message}`,
        });
    }
  },
);
